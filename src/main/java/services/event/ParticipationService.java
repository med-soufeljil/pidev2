...
/**
 * Service pour la gestion des participations aux événements
 * Implémente l'interface IService pour les opérations CRUD sur les participations
 */
public class ParticipationService implements IService<Participation> {
     ...
     * Vérifie d'abord les contraintes (places disponibles, date non passée)
     * @param p La participation à ajouter
     * @throws SQLException en cas d'erreur SQL ou de violation des contraintes
     */
    @Override
    public void ajouter(Participation p) throws SQLException {
        ...
        ps.setDate(5, Date.valueOf(LocalDate.now())); // Date de création = aujourd'hui
        ps.executeUpdate();

        // Mise à jour du compteur d'inscrits dans l'événement
        mettreAJourNombreInscrits(p.getId_e(), +1);
    }

    /**
     * Modifie une participation existante
     * @param p La participation avec les nouvelles données
    ...
    public void modifier(Participation p) throws SQLException {
        ...
        ps.setString(3, p.getStatut());
        ps.setBoolean(4, p.isPresence());
        ps.setDate(5, Date.valueOf(LocalDate.now())); // Date de modification = aujourd'hui
     ...
     * @param p La participation à supprimer
     * @throws SQLException en cas d'erreur SQL
     */
    ...
    public void supprimer(Participation p) throws SQLException {
        ...
        // Mise à jour du compteur d'inscrits (décrémentation)
        mettreAJourNombreInscrits(idEvent, -1);
    }

    /**
     * Récupère toutes les participations avec les informations des événements associés
    ...
    public List<Participation> recuperer() throws SQLException {
        ...
        while (rs.next()) {
            try {
                ...
                // Récupération des dates avec gestion des valeurs nulles ou invalides
                try {
                ...
                // Créer et set l'objet Evenement associé
    ...
    /**
     * Vérifie si on peut ajouter une participation (places disponibles)
     * @param idEvent L'ID de l'événement
     * @return true si on peut ajouter, false sinon
     * @throws SQLException en cas d'erreur SQL
     */
    ...
    /**
     * Vérifie si la date d'inscription n'est pas après la date de l'événement
     * @param idEvent L'ID de l'événement
    ...
    /**
     * Met à jour le nombre d'inscrits dans la table evenements
     * @param idEvent L'ID de l'événement
     * @param increment La valeur à ajouter (positif ou négatif)
     * @throws SQLException en cas d'erreur SQL
     */
