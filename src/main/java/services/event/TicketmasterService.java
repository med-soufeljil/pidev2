...
/**
 * Service d'intégration avec l'API Ticketmaster
 * Permet de rechercher des événements culturels et sportifs
 */
public class TicketmasterService {

    // 🔴 REMPLACE PAR TA VRAIE CLÉ API (obtenue sur developer.ticketmaster.com)
    private static final String API_KEY = "z0Ivhuv6gXDrbTfkS1I8EPeAGOlmQqRe";

    private static final String BASE_URL = "https://app.ticketmaster.com/discovery/v2";

    private final OkHttpClient client;        // Client HTTP
    private final ObjectMapper mapper;         // Mapper JSON
    private final Random random;                // Générateur aléatoire (pour données fictives)
     ...
     * @param keyword Mot-clé (concert, sport, etc.)
     * @param city Ville
     * @param countryCode Code pays (TN, FR, etc.)
     * @return Liste d'événements trouvés
     */
    public List<TicketmasterEvent> searchEvents(String keyword, String city, String countryCode) {
        ...
        try {
            ...
            urlBuilder.append("&size=20"); // Limite à 20 résultats
            ...
            if (countryCode != null && !countryCode.isEmpty()) {
                ...
                urlBuilder.append("&countryCode=TN,FR,IT,ES"); // Par défaut: Tunisie + Europe
            }
        ...
        // Si aucun événement trouvé, générer des données fictives pour la démonstration
        if (events.isEmpty()) {
     ...
     * Utilisé quand l'API n'est pas disponible ou pas configurée
     */
    ...
    /**
     * Classe interne représentant un événement Ticketmaster
     */
    public static class TicketmasterEvent {
        ...
        /**
         * Affichage court pour le chatbot (liste)
         */
        ...
        /**
         * Affichage détaillé pour le chatbot
         */
