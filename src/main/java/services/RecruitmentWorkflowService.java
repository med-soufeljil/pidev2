...
public class RecruitmentWorkflowService {
    ...
    public String getCandidatePhase(int candidateId) throws SQLException {
        ...
        if (hasReunion) return "INTERVIEW_SCHEDULED";
        if (hasRecrutement) return "SHORTLISTED";
        return "APPLIED";
    }
    ...
    private void saveGeneratedOffer(int candidateId, String generatedOffer) throws SQLException {
        String sql = """
                ...
                VALUES (?, 'OFFER_SENT', ?)
                ON DUPLICATE KEY UPDATE phase='OFFER_SENT', generatedOffer=VALUES(generatedOffer)
                """;
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, candidateId);
            ps.setString(2, generatedOffer);
            ps.executeUpdate();
        }
    }
